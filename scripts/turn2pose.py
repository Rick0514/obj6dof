...
for each in T_str:
    ...
    h_rot0 = np.eye(4)
    h_rot0[:3, :3] = rot0
    hq = tfs.quaternion_from_matrix(h_rot0)

    theta = 2 * math.acos(hq[3])
    rotv = theta / math.sin(theta / 2) * hq[:3]

    print(rotv)
    res.write('{} {} {} '.format(*rotv))

    # trans
    xyz = list(100 * trans0.reshape(3,))    # cm
    res.write('{} {} {}\n'.format(*xyz))

    T.append([hq, trans0.reshape(3,)])
