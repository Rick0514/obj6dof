...
namespace gazebo
{
GZ_REGISTER_MODEL_PLUGIN(SimP3D);
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
void SimP3D::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    // Get the world name.
    ...
    if(_sdf->HasElement("tf_on"))
        this->tf_on_ = _sdf->GetElement("tf_on")->Get<bool>();
    ...
    this->callback_queue_thread_ = boost::thread(
        boost::bind(&SimP3D::P3DQueueThread, this));

    // New Mechanism for Updating every World Cycle
    // Listen to the update event. This event is broadcast every
    // simulation iteration.
    this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
        boost::bind(&SimP3D::UpdateChild, this));
}

////////////////////////////////////////////////////////////////////////////////
// Update the controller
void SimP3D::UpdateChild()
{
    if (!this->link_)
        return;
  ...
  // rate control
    ...
    if (tmp_dt != 0)
    {
        ...
        // Apply Reference Frame
            ...
            // pub tf
            tf::Transform tf;
            geometry_msgs::Pose odom_pose = this->pose_msg_.pose.pose;
            tf::poseMsgToTF(odom_pose, tf);
            tf::StampedTransform stamped_tf(tf, pose_msg_.header.stamp, frame_name_, link_name_);
            this->tf_br_.sendTransform(stamped_tf);
...
double SimP3D::GaussianKernel(double mu, double sigma)
{
    if(ignition::math::equal(sigma, 0.0))   return 0.0;
